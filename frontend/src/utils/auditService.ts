...
class AuditService {
  ...
  async getAuditStats(): Promise<{
    ...
    try {
      ...
      return {
        totalLogs: parseInt(totalResult.rows[0]?.total || '0'),
        successfulActions: parseInt(successResult.rows[0]?.successful || '0'),
        failedActions: parseInt(failedResult.rows[0]?.failed || '0'),
