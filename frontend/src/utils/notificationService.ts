...
const NOTIFICATIONS_STORAGE_KEY = 'medstaff_notifications'
...
const loadNotificationsFromStorage = (): Notification[] => {
  ...
  // Se não há dados salvos ou houve erro, retorna os dados iniciais
  const notifications = [...initialMockNotifications]
  saveNotificationsToStorage(notifications)
  return notifications
}

// Carrega as notificações do localStorage ou usa os dados iniciais
let mockNotifications = loadNotificationsFromStorage()
  ...
  subscribe(callback: (notification: Notification) => void) {
    ...
    if (!this.interval) {
      this.interval = setInterval(() => {
        ...
        // Adiciona a nova notificação ao array e persiste
        mockNotifications.unshift(randomNotification)
...
export const useNotifications = () => {
  ...
  const markAsRead = useCallback(async (notificationId: string) => {
    try {
      ...
      // Recarregar stats sem dependência circular
  ...
  useEffect(() => {
    const unsubscribe = notificationManager.subscribe(async (notification) => {
