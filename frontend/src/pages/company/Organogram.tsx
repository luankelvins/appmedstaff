...
import { employeeDataService, EmployeeHierarchy, OrganogramData } from '../../utils/employeeDataService'
...
function convertToEmployee(emp: EmployeeHierarchy): Employee {
  return {
    id: emp.id,
    name: emp.nome,
    position: emp.cargo,
    department: emp.departamento,
    manager: emp.gestor_id || undefined,
    email: emp.email,
    phone: emp.dados_pessoais?.telefone || '',
    subordinates: emp.subordinados ? emp.subordinados.map(convertToEmployee) : [],
    
    // Dados adicionais
    nivel_hierarquico: emp.nivel_hierarquico,
    ordem_organograma: emp.ordem_organograma,
    status: emp.status,
    created_at: emp.created_at,
    foto: emp.dados_pessoais?.foto,
    data_admissao: emp.dados_profissionais?.data_admissao,
    gestor_nome: emp.gestor_nome,
    gestor_cargo: emp.gestor_cargo
  }
}
...
export const Organogram: React.FC = () => {
  ...
  // Carregar dados do organograma
  const loadOrganogramData = async () => {
    try {
      setLoading(true)
      setError(null)
      const data = await employeeDataService.getOrganogramData()
      setOrganogramData(data)
      
      // Converter para formato do componente
      const convertedEmployees = [convertToEmployee(data.ceo)]
      setEmployees(convertedEmployees)
    } catch (err) {
      console.error('Erro ao carregar dados do organograma:', err)
      setError('Erro ao carregar dados do organograma')
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg p-4 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">{organogramData?.totalEmployees || 0}</p>
            </div>
        ...
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-4 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">{organogramData ? Object.keys(organogramData.departmentCounts).length : 0}</p>
            </div>
        ...
        <div className="bg-gradient-to-r from-green-500 to-green-600 rounded-lg p-4 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">{organogramData ? Object.values(organogramData.hierarchyLevels).filter(level => level.length > 0 && level[0].nivel_hierarquico > 1).reduce((acc, level) => acc + level.length, 0) : 0}</p>
            </div>
        ...
        <div className="bg-gradient-to-r from-orange-500 to-orange-600 rounded-lg p-4 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">{organogramData ? Object.keys(organogramData.hierarchyLevels).length : 0}</p>
            </div>
