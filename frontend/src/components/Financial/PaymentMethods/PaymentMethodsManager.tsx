...
import { Plus, Search, Edit, Trash2, CreditCard, Banknote, Smartphone, FileText, Building2, MoreHorizontal, AlertCircle } from 'lucide-react';
...
const PaymentMethodsManager: React.FC<PaymentMethodsManagerProps> = ({ 
  ...
  const loadMethods = async () => {
    ...
    setError(null);
    try {
      const data = await financialService.getPaymentMethods();
      setMethods(data);
    } catch (error) {
  ...
  const handleDeleteMethod = async (methodId: string) => {
    if (window.confirm('Tem certeza que deseja excluir esta forma de pagamento?')) {
      try {
        ...
        setError('Erro ao excluir forma de pagamento. Tente novamente.');
      }
    }
  };
  ...
  const handleSaveMethod = async (formData: PaymentMethodFormData) => {
    try {
      if (editingMethod) {
        const updatedMethod = await financialService.updatePaymentMethod(editingMethod.id, formData);
        ...
        const newMethod = await financialService.createPaymentMethod(methodData);
      ...
      setError('Erro ao salvar forma de pagamento. Tente novamente.');
    }
  };
