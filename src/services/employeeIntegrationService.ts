...
export const employeeIntegrationService = {
  /**
   * Inicializa dados de exemplo no Supabase (apenas para desenvolvimento)
   */
  async initializeSampleData(): Promise<void> {
    try {
      const existingEmployees = await supabaseService.getAllEmployees()
  ...
  async getEmployeeDataByEmail(email: string): Promise<EmployeeProfileIntegration> {
    try {
      // Busca membro do time interno pelo email pessoal no Supabase
      const employeeData = await supabaseService.getEmployeeByEmail(email)
  ...
  async getEmployeeById(id: string): Promise<TimeInternoForm | null> {
    try {
      // Por enquanto, busca todos e filtra por ID (pode ser otimizado no futuro)
  ...
  async updateEmployeeData(id: string, data: Partial<TimeInternoForm>): Promise<TimeInternoForm | null> {
    try {
      // Atualiza usando upsert (pode ser melhorado com um método específico de update)
      const updatedData = { ...data, id }
      return await supabaseService.upsertEmployee(updatedData as TimeInternoForm)
    } catch (error) {
