...
COMMENT ON TABLE employees IS 'Tabela de funcionários - contém dados pessoais, profissionais e financeiros dos membros da equipe';
COMMENT ON TABLE tasks IS 'Tabela de tarefas - sistema de gerenciamento de tarefas da aplicação';
COMMENT ON TABLE leads IS 'Tabela de leads - prospects e potenciais clientes';
COMMENT ON TABLE clientes_pf IS 'Tabela de clientes pessoa física';
COMMENT ON TABLE clientes_pj IS 'Tabela de clientes pessoa jurídica';
COMMENT ON TABLE contratos IS 'Tabela de contratos - documentos contratuais com clientes';
COMMENT ON TABLE irpf IS 'Tabela de declarações de imposto de renda pessoa física';