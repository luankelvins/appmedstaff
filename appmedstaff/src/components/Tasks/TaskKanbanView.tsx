...
const TaskKanbanView: React.FC<TaskKanbanViewProps> = ({ onTaskClick, onCommentsClick }) => {
  ...
  const loadTasks = async () => {
    try {
      ...
      const response = await taskService.getTasks({}, { field: 'updatedAt', direction: 'desc' }, 1, 100);
  ...
  const handleFormSubmit = async (data: CreateTaskRequest | UpdateTaskRequest) => {
    try {
      ...
      if (editingTask) {
        ...
        await taskService.createTask(createData, 'current-user'); // TODO: Get from auth context
      }
